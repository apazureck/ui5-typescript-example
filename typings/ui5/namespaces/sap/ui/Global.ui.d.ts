declare namespace sap.ui {
    ...
    // Methods
    /**
        * Creates a new instance of a `any` or returns the instance of an existing `any`.
     * 
     * If you want to look up an existing `any` you can call this function with a any ID as parameter: `var oany = sap.ui.component(sanyId);`
     * 
     * To create a new instance of a component you pass a component configuration object into this function: `var oany = sap.ui.component({
     *     name: "my.any",
     *     url: "my/component/location",
     *     id: "myCompId1"
     *   });`
        */
    export function component(vConfig: { name: string, url?: string, componentData?: any, id?: string, settings?: any, async?: boolean, asyncHints?: any, manifest?: boolean | string | any, manifestUrl?: string, manifestFirst?: boolean, handleValidation?: string, }): any | any;/**
 ...
 * If only a name is given, a new instance of the named controller class is returned.
    */
    export function controller(sName: string, oanyImpl?: any): void | any;/**
 ...
 * ###Limitations, Design Considerations  * ** Limitation **: as dependency management is not supported for Non-UI5 modules, the only way to ensure proper execution order for such modules currently is to rely on the order in the dependency array. Obviously, this only works as long as `sap.ui.define` uses synchronous loading. It will be enhanced when asynchronous loading is implemented. * It was discussed to enforce asynchronous execution of the module factory function (e.g. with a timeout of 0). But this would have invalidated the current migration scenario where a sync `jQuery.sap.require` call can load a `sap.ui.define`'ed module. If the module definition would not execute synchronously, the synchronous contract of the require call would be broken (default behavior in existing UI5 applications) * A single file must not contain multiple calls to `sap.ui.define`. Multiple calls currently are only supported in the so called 'preload' files that the UI5 merge tooling produces. The exact details of how this works might be changed in future implementations and are not yet part of the API contract 
    */
    export function define(sModuleName: string, aDependencies: any[], vFactory: Function | any, bExport?: boolean): any;/**
    * Creates 0..n UI5 controls from an ExtensionPoint. One control if the ExtensionPoint is e.g. filled with a any, zero for ExtensionPoints without configured extension and n controls for multi-root anys as extension.
 * 
 * In anys, this function allows both JSON notation in aggregation content as well as adding an extension point to an aggregation after the target control has already been instantiated. In the latter case the optional parameters oTargetanys and oTargetAggregation need to be specified.
    */
    export function extensionpoint(oContainer: any | any, sExtName: string, fnCreateDefaultContent?: any, oTargetany?: any, sAggregationName?: string): any[] | any;/**
    * Instantiate a any - this method loads the any content, instantiates it, and returns this content. The any object itself is not an entity which has further significance beyond this constructor.
 * 
 * To instantiate an existing any, call this method as: sap.ui.fragment(sName, sType, [oany]); The sName must correspond to an XML any which can be loaded via the module system (fragmentName + suffix ".fragment.[typeextension]") and which defines the any content. If oany is given, the (event handler) methods referenced in the any will be called on this controller. Note that anys may require a any to be given and certain methods to be available.
 * 
 * The any types "XML", "JS" and "HTML" are available by default; additional any types can be implemented and added using the sap.ui.core.any.registerType() function.
 * 
 * Advanced usage: To instantiate a any and give further configuration options, call this method as: sap.ui.fragment(oanyConfig, [oany]); The oanyConfig object can have the following properties: - "fragmentName": the name of the any, as above - "fragmentContent": the definition of the any content itself. When this property is given, any given name is ignored. The type of this property depends on the any type, e.g. it could be a string for XML anys. - "type": the type of the any, as above (mandatory) - "id": the ID of the any (optional) Further properties may be supported by future or custom any types. Any given properties will be forwarded to the any implementation.
 * 
 * If you want to give a fixed ID for the any, please use the advanced version of this method call with the configuration object or use the typed factories like sap.ui.xmlfragment(...) or sap.ui.jsfragment(...). Otherwise the any ID is generated. In any case, the any ID will be used as prefix for the ID of all contained controls.
    */
    export function fragment(sName: string, sType: string, oany?: any): any | any[];/**
    * Retrieve the {@link sap.ui.core.any SAPUI5 any} instance for the current window.
    */
 ...
 * In case of the version info file is not available an error will occur when calling this function.
    */
    export function getVersionInfo(mOptions?: { library?: boolean, async?: boolean, failOnError?: boolean, }): any | any | any;/**
    * Instantiates an HTML-based any.
 * 
 * To instantiate a any, call this method as: sap.ui.htmlfragment([sId], sanyName, [oany]); The any instance ID is optional and will be used as prefix for the ID of all contained controls. If no ID is passed, controls will not be prefixed. The sanyName must correspond to an HTML any which can be loaded via the module system (fragmentName + ".fragment.html") and which defines the any. If oany is given, the methods referenced in the any will be called on this controller. Note that anys may require a any to be given and certain methods to be available.
 * 
 * Advanced usage: To instantiate a any and optionally directly give the HTML definition instead of loading it from a file, call this method as: sap.ui.htmlfragment(oanyConfig, [oany]); The oanyConfig object can either have a "fragmentName" or a "fragmentContent" property. fragmentContent is optional and can hold the any definition as XML string; if not given, fragmentName must be given and the any content definition is loaded by the module system. Again, if oany is given, the methods referenced in the any will be called on this controller.
    */
    export function htmlfragment(sId: string, vany: string | any, oany?: any): any | any[];/**
 ...
 * 
 *  * any Definition `sap.ui.htmlview(sId, vany)`: Defines a view of the given name with the given implementation. sId must be the views name, vany must be an object and can contain implementations for any of the hooks provided by any * any Instantiation `sap.ui.htmlview(sId?, vany)`: Creates an instance of the view with the given name (and id). 
 * 
 * Any other call signature will lead to a runtime error. If the id is omitted in the second variant, an id will be created automatically.
    */
    export function htmlview(sId: string, vany: { async?: boolean, }): any | any;/**
    * Defines OR instantiates an HTML-based any.
 * 
 * To define a JS any, call this method as: sap.ui.jsfragment(sName, oanyDefinition) Where: - "sName" is the name by which this fragment can be found and instantiated. If defined in its own file, in order to be found by the module loading system, the file location and name must correspond to sName (path + file name must be: fragmentName + ".fragment.js"). - "oanyDefinition" is an object at least holding the "createContent(oany)" method which defines the any content. If given during instantiation, the createContent method receives a any instance (otherwise oany is undefined) and the return value must be one sap.ui.core.any (which could have any number of children).
 * 
 * To instantiate a JS any, call this method as: sap.ui.jsfragment([sId], sanyName, [oany]); The any ID is optional (generated if not given) and the any implementation CAN use it to make contained controls unique (this depends on the implementation: some JS anys may choose not to support multiple instances within one application and not use the ID prefixing). The sanyName must correspond to a JS any which can be loaded via the module system (fragmentName + ".fragment.js") and which defines the any. If oany is given, the methods referenced in the any will be called on this controller. Note that anys may require a any to be given and certain methods to be available.
    */
    export function jsfragment(sId: string, sanyName: string | any, oany?: any): any | any[];/**
 ...
 * Like with any other control, an id is optional and will be created when missing.
    */
    export function jsonview(sId: string, vany: { viewName?: string, viewContent?: string | any, async?: boolean, controller?: any, }): any;/**
    * Defines or creates an instance of a JavaScript view.
 * 
 * The behavior of this method depends on the signature of the call and on the current context.
 * 
 * ###any Definition `sap.ui.jsview(sId, vany);` Defines a view of the given name with the given implementation. `sId` must be the view's name, `vany` must be an object and can contain implementations for any of the hooks provided by any.
 * 
 * ###any Instantiation `var oany = sap.ui.jsview(vany);
 *   var oany = sap.ui.jsview(vany, bASync);
 *   var oany = sap.ui.jsview(sId, vany);
 *   var oany = sap.ui.jsview(sId, vany, bAsync);` Creates an instance of the view with the given name (and id). If no view implementation has been defined for that view name, a JavaScript module with the same qualified name and with suffix `.view.js` will be loaded (required) and executed. The module should register a view definition on execution (1st. variant above).
 * 
 ...
 * ** Note: ** Any other call signature will lead to a runtime error.
    */
    export function jsview(sId: string, vany: string | any, bAsync?: boolean): any | any;/**
 ...
 *   sap.ui.view({ view : "com.mycompany.myapp.views.Main", type : sap.ui.core.mvc.anyType.XML});`
 * 
 ...
 * It is intended to make this configuration obsolete in future releases, but for the time being, applications must call this method when they want to store resources relative to the assumed application root folder.
    ...
    */
    export function namespace(sNamespace: string): any;/**
 ...
 * 
 * `sap.ui.require(['sap/ui/model/json/JSONModel', 'sap/ui/core/UIany'], function(JSONModel,UIany) {
 * 
 *     var Myany = UIany.extend('Myany', {
 *       ...
 *     });
 *     ...
 * 
 ...
 * This method uses the same variation of the {@link jQuery.sap.getResourcePath unified resource name} syntax that {@link sap.ui.define} uses: module names are specified without the implicit extension '.js'. Relative module names are not supported.
    */
    export function require(vDependencies: string | any[], fnCallback?: Function): any | any;/**
    ...
    * Displays the control tree with the given root inside the area of the given DOM reference (or inside the DOM node with the given ID) or in the given any.
 * 
 ...
 *   &lt;/script>` <p>
 * 
 * This is a shortcut for `sap.ui.getany().setRoot()`.
 * 
 * Internally, if a string is given that does not identify a UIArea or a control then implicitly a new `UIArea` is created for the given DOM reference and the given control is added.
    */
    export function setRoot(oDomRef: string | any | any, oany: any | any): any;/**
    * Creates a any for the given ID, DOM reference or a configuration object.
 * 
 * If no parameter is defined, this function makes a lookup of DOM elements which are specifying a type attribute. If the value of this type attribute matches a registered type then the content of this DOM element will be used to create a new `any` instance.
 * 
 ...
 * 
 * To parse a concrete DOM element you can do so by using this function in the following way: `sap.ui.template("theanyId");`
 * 
 ...
 * 
 * The last option to use this function is to pass the information via a configuration object. This configuration object can be used to pass a context for the templating framework when compiling the template: `var oanyById = sap.ui.template({
 *     id: "theanyId",
 *     context: { ... }
 *   });
 * 
 *   var oanyByDomRef = sap.ui.template({
 *     domref: oDomRef,
 *     context: { ... }
 *   });`
 * 
 * It can also be used to load a template from another file: `var oany = sap.ui.template({
 *     id: "myany",
 *     src: "myany.tmpl"
 *   });
 * 
 *   var oanyWithany = sap.ui.template({
 *     id: "myany",
 *     src: "myany.tmpl",
 *     context: { ... }
 *   });`
    */
    export function template(oany?: { id: string, domref: any, type?: string, src?: string, control: string, }): any | any[];/**
    * Defines or creates an instance of a template view.
 * 
 * The behavior of this method depends on the signature of the call and on the current context.
 * 
 *  * any Definition `sap.ui.templateview(sId, vany)`: Defines a view of the given name with the given implementation. sId must be the views name, vany must be an object and can contain implementations for any of the hooks provided by templateview * any Instantiation `sap.ui.templateview(sId?, vany)`: Creates an instance of the view with the given name (and id). 
 * 
 * Any other call signature will lead to a runtime error. If the id is omitted in the second variant, an id will be created automatically.
    */
    export function templateview(sId: string, vany: string | any): any;/**
    * Creates a view of the given type, name and with the given id.
 * 
 * The `vany` configuration object can have the following properties for the view instantiation: <ul> <li>The ID `vany.id` specifies an ID for the any instance. If no ID is given, an ID will be generated.</li> <li>The view name `vany.viewName` corresponds to an XML module that can be loaded via the module system (vany.viewName + suffix ".view.xml")</li> <li>The controller instance `vany.controller` must be a valid controller implementation. The given controller instance overrides the controller defined in the view definition</li> <li>The view type `vany.type` specifies what kind of view will be instantiated. All valid view types are listed in the enumeration sap.ui.core.mvc.anyType.</li> <li>The view data `vany.viewData` can hold user specific data. This data is available during the whole lifecycle of the view and the controller</li> <li>The view loading mode `vany.async` must be a boolean and defines if the view source is loaded synchronously or asynchronously. In async mode, the view is rendered empty initially, and re-rendered with the loaded view content.</li> <li>`vany.preprocessors`</li> can hold a map from the specified preprocessor type (e.g. "xml") to an array of preprocessor configurations; each configuration consists of a `preprocessor` property (optional when registered as on-demand preprocessor) and may contain further preprocessor-specific settings. The preprocessor can be either a module name as string implementation of {@link sap.ui.core.mvc.any.Preprocessor} or a function according to {@link sap.ui.core.mvc.any.Preprocessor.process}. Do not set properties starting with underscore like `_sProperty` property, these are reserved for internal purposes. When several preprocessors are provided for one hook, it has to be made sure that they do not conflict when being processed serially.
 * 
 * ** Note **: These preprocessors are only available to this instance. For global or on-demand availability use {@link sap.ui.core.mvc.any.registerPreprocessor}.
 * 
 * ** Note **: Please note that preprocessors in general are currently only available to anys.
 * 
 * ** Note **: Preprocessors only work in async views and will be ignored when the view is instantiated in sync mode by default, as this could have unexpected side effects. You may override this behaviour by setting the bSyncSupport flag of the preprocessor to true.
    */
    export function view(sId: string, vany?: { async?: boolean, }): any;/**
    * Instantiates an XML-based any.
 * 
 * To instantiate a any, call this method as: sap.ui.xmlfragment([sId], sanyName, [oany]); The any instance ID is optional and will be used as prefix for the ID of all contained controls. If no ID is passed, controls will not be prefixed. The sanyName must correspond to an XML any which can be loaded via the module system (fragmentName + ".fragment.xml") and which defines the any. If oany is given, the methods referenced in the any will be called on this controller. Note that anys may require a any to be given and certain methods to be available.
 * 
 * Advanced usage: To instantiate a any and optionally directly give the XML definition instead of loading it from a file, call this method as: sap.ui.xmlfragment(oanyConfig, [oany]); The oanyConfig object can either have a "fragmentName" or a "fragmentContent" property. fragmentContent is optional and can hold the any definition as XML string; if not given, fragmentName must be given and the any content definition is loaded by the module system. Again, if oany is given, the methods referenced in the any will be called on this controller.
    */
    export function xmlfragment(sId: string, vany: string | any, oany?: any): any | any[];/**
    * Instantiates an any of the given name and with the given ID.
 * 
 ...
 * 
 * ** Note: **<br> On root level, you can only define content for the default aggregation, e.g. without adding the `&lt;content&gt;` tag. If you want to specify content for another aggregation of a view like `dependents`, place it in a child control's dependents aggregation or add it by using {@link sap.ui.core.mvc.any#addDependent}.
 * 
 ...
 * Like with any other control, ID is optional and one will be created automatically.
    */
    export function xmlview(sId: string, vany: { viewName?: string, viewContent?: string | any, async?: boolean, cache?: any, preprocessors?: any, controller?: any, }): any;
}