    ...
    */
    export default class GenericTile extends Control {
        ...
        */
         ...
         * The event is triggered when the user presses the tile.
        */
        attachPress<TcustomData, Tcontext>(oData: any, fnFunction: (this: Tcontext, oEvent: Event<this, { /* * The current scope the GenericTile was in when the event occurred. */
            scope: GenericTileScope, /* * The action that was pressed on the tile. In the Actions scope, the available actions are Press and Remove. In Display scope, the parameter value is only Press. */
            action: string, /* * The pressed DOM Element pointing to the GenericTile's DOM Element in Display scope. In Actions scope it points to the more icon, when the tile is pressed, or to the DOM Element of the remove button, when the remove button is pressed. */
            domRef: any,
